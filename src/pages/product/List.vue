...
export default {
  ...
  methods:{
    ...
    loadCategory(){
      ...
      let url = "http://101.132.136.165:6677/product/findAll"
    ...
    loadData(){
      this.fileList = []
      let url = "http://101.132.136.165:6677/product/query"
      request({
          url,
          method:"post",
          headers:{
    ...
    submitHandler(){
      ...
      this.fileList=[]
      let url = "http://101.132.136.165:6677/product/saveOrUpdate";
      request({
        url,
        method:"POST",
        headers:{
    ...
    toDeleteHandler(id){
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        // 调用后台接口，完成删除操作
        let url = "http://101.132.136.165:6677/product/deleteById?id="+id;
        request.get(url).then((response)=>{
          //1. 刷新数据
          this.loadData();
          //2. 提示结果
